	...
	return (
		<div className="min-h-screen bg-background text-foreground">
			...
			<section className="relative overflow-hidden px-4 py-20 ambient-glow sm:px-6 md:py-32">
				<div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_16%_20%,rgba(169,198,82,0.15),transparent_28%),radial-gradient(circle_at_85%_20%,rgba(98,98,198,0.08),transparent_26%)]" />
				<div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
					...
					<motion.div
						...
						className="rounded-[1.75rem] border border-border bg-card/90 p-4 shadow-2xl shadow-primary-dark/10 dark:bg-card/75 sm:p-5 md:rounded-[2rem]"
					>
						<div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950 p-4 font-mono text-xs leading-7 text-slate-300 sm:p-5 sm:text-sm">
							...
							<p className="whitespace-nowrap">base_url=<span className="text-primary">"https://api.koreshield.com/v1"</span></p>
						</div>
						<div className="mt-4 grid gap-3 sm:grid-cols-3">
							{steps.map(([number, title, body]) => (
								<div key={title} className="rounded-2xl border border-border bg-background/70 p-4">
									<p className="text-xs font-bold tracking-[0.22em] text-primary">{number}</p>
			...
			<section className="border-y border-border bg-card/35 px-4 py-16 sm:px-6 md:py-20">
				<div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
					<div>
						<p className="mb-4 text-xs font-bold uppercase tracking-[0.3em] text-primary">Where it works</p>
