	...
	return (
		<div className="min-h-screen bg-background text-foreground">
			...
			<section className="relative overflow-hidden px-4 py-20 ambient-glow sm:px-6 md:py-28">
				<div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_18%,rgba(169,198,82,0.16),transparent_30%),radial-gradient(circle_at_78%_14%,rgba(98,98,198,0.08),transparent_24%)]" />
				<div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
					<motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
						<span className="mb-5 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.24em] text-primary sm:text-xs">
						...
						<div className="mt-8 flex flex-col gap-3 sm:flex-row">
							<Link to="/signup?plan=growth" className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-primary-dark sm:px-7">
					...
					<motion.div
						...
						className="rounded-[1.75rem] border border-border bg-card/85 p-4 shadow-2xl shadow-primary-dark/10 sm:p-5 md:rounded-[2rem]"
					>
						...
						<div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
							{flow.map((item, index) => (
								<div key={item} className="rounded-2xl border border-border bg-background/70 p-4">
									<p className="text-[10px] font-black uppercase tracking-[0.24em] text-primary">0{index + 1}</p>
				...
				<div className="mx-auto grid max-w-7xl gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{solutions.map((solution, index) => (
						<motion.article
							...
							className="group rounded-[1.5rem] border border-border bg-card/85 p-5 shadow-sm transition-all hover:-translate-y-1 hover:border-primary/30 md:rounded-[2rem] md:p-6"
						>
							...
							<Link to={solution.to} className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-primary hover:underline">
			...
			<section className="px-4 py-16 sm:px-6 md:py-20">
				<div className="mx-auto flex max-w-7xl flex-col gap-5 rounded-[1.75rem] border border-border bg-card/85 p-6 shadow-sm md:flex-row md:items-center md:justify-between md:rounded-[2rem] md:p-8">
					<div>
						<p className="mb-3 text-xs font-bold uppercase tracking-[0.26em] text-primary">Not sure where to start?</p>
						<h2 className="text-3xl font-extrabold tracking-[-0.04em] md:text-4xl">Start with the proxy. Expand as the risk becomes real.</h2>
					</div>
					<Link to="/solutions/ai-application-protection" className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-primary-dark">
