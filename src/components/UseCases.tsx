...
const useCases = [
	...
	},
	{
		...
		tag: 'Multi-tenant AI',
		tagColor: 'text-secondary-dark bg-secondary-dark/10 border-secondary-dark/20',
	},
	{
		icon: Scale,
				...
				<div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
					{useCases.map((useCase, index) => (
						<motion.div
							...
							viewport={{ once: true }}
							whileHover={{ y: -4 }}
							className="group relative flex min-h-[300px] flex-col justify-between overflow-hidden rounded-[1.75rem] border border-white/[0.08] bg-card/80 p-7 shadow-sm transition-all duration-300 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5"
						>
							<div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent" />
							<div className="flex items-start justify-between">
								<div className="bg-primary/10 w-14 h-14 rounded-xl flex items-center justify-center group-hover:bg-primary/20 transition-colors shrink-0">
